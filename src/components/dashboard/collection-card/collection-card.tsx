...
export const CollectionCard = ({ collection, authenticated = false, onDelete }: Props) => {
    return (
        <div className={styles.collection_card}>
            ...
            <Paragraph>
                {collection.created_at ? `Créée le ${dayjs(collection.created_at).format('DD/MM/YYYY')}` : ''}
            </Paragraph>
            <Paragraph>
                {collection.updated_at ? `Modifiée le ${dayjs(collection.updated_at).format('DD/MM/YYYY')}` : ''}
            </Paragraph>
            <Badge>
